...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  <xsl:template match="/configuration">
    ...
    COMMON_NAME="<xsl:valueof select="credentials/application/productId"/>:<xsl:valueof select="device/serialNumber"/>" openssl req -new -key certs/client/client.key -out certs/client/client.csr -config ssl.cnf -sha256
    ...
    openssl pkcs12 -inkey certs/client/client.key -in certs/client/client.crt -export -out certs/client/client.pkcs12 -password pass:<xsl:valueof select="device/keystore/password"/>
    ...
    openssl pkcs12 -inkey certs/server/jetty.key -in certs/server/jetty.crt -export -out certs/server/jetty.pkcs12 -password pass:<xsl:valueof select="device/keystore/password"/>
